...
class RemBertLayerBetterTransformer(BetterTransformerBaseLayer):
    def __init__(self, rembert_layer, config):
        r"""
        A simple conversion of the RemBert layer to its `BetterTransformer` implementation.

        Args:
            rembert_layer (`torch.nn.Module`):
                The original RemBert layer where the weights needs to be retrieved.
        """
        super().__init__(config)
        # In_proj layer
        self.in_proj_weight = nn.Parameter(
            torch.cat(
                [
                    rembert_layer.attention.self.query.weight,
                    rembert_layer.attention.self.key.weight,
                    rembert_layer.attention.self.value.weight,
                ]
            )
        )
        ...
        self.in_proj_bias = nn.Parameter(
            torch.cat(
                [
                    rembert_layer.attention.self.query.bias,
                    rembert_layer.attention.self.key.bias,
                    rembert_layer.attention.self.value.bias,
                ]
            )
        )

        # Out proj layer
        self.out_proj_weight = rembert_layer.attention.output.dense.weight
        self.out_proj_bias = rembert_layer.attention.output.dense.bias

        # Linear layer 1
        self.linear1_weight = rembert_layer.intermediate.dense.weight
        self.linear1_bias = rembert_layer.intermediate.dense.bias

        # Linear layer 2
        self.linear2_weight = rembert_layer.output.dense.weight
        self.linear2_bias = rembert_layer.output.dense.bias

        # Layer norm 1
        self.norm1_eps = rembert_layer.attention.output.LayerNorm.eps
        self.norm1_weight = rembert_layer.attention.output.LayerNorm.weight
        self.norm1_bias = rembert_layer.attention.output.LayerNorm.bias

        # Layer norm 2
        self.norm2_eps = rembert_layer.output.LayerNorm.eps
        self.norm2_weight = rembert_layer.output.LayerNorm.weight
        self.norm2_bias = rembert_layer.output.LayerNorm.bias

        # Model hyper parameters
        self.num_heads = rembert_layer.attention.self.num_attention_heads
        self.embed_dim = rembert_layer.attention.self.all_head_size
